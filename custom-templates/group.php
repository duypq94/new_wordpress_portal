<?php 
	/* 
	Template Name: group
	*/
?>
<?php get_header();?>
<div class="container-fluid main-content">
    <div class="container main-page group-page">
        <div class="card card-js">
            <div class="card-title">
                <i class="fas fa-building"></i>株式会社メディアドゥホールディングス
            </div>
            <div class="card-content">
                <div class="tab">
                    <button class="tablinksGroupPage tablinks" onclick="openTabGroupPage(event, 'GroupPageId101')"
                        id="defaultOpen-group-page">会社情報</button>
                    <button class="tablinksGroupPage tablinks"
                        onclick="openTabGroupPage(event, 'GroupPageId102')">グループ会社一覧</button>
                </div>


                <div id="GroupPageId101" class="tabcontentGroupPage tabcontent">
                    <table class="group-page-table" style="border-bottom:1px solid #ccc;">
                        <tbody>
                            <tr>
                                <td width="110px">会社名</td>
                                <td>株式会社メディアドゥホールディングス／MEDIA DO HOLDINGS Co., Ltd.</td>
                            </tr>
                            <tr>
                                <td>所在地</td>
                                <td>
                                    <table>
                                        <tr>
                                            <td>本社：</td>
                                            <td>〒100-0003 東京都千代田区一ツ橋1-1-1 パレスサイドビル 5F</td>
                                        </tr>
                                        <tr>
                                            <td>名古屋オフィス：</td>
                                            <td>〒460-0002 名古屋市中区丸の内3-5-10 名古屋丸の内ビル 9F</td>
                                        </tr>
                                    </table>
                                </td>
                            </tr>

                            <tr>
                                <td>創業／設立</td>
                                <td>1996年4月1日／1999年4月1日</td>
                            </tr>
                            <tr>
                                <td>資本金</td>
                                <td>957,404,750円 <small><span>※2018年8月31日時点</span></small></td>
                            </tr>
                            <tr>
                                <td>役員</td>
                                <td>
                                    <table>
                                        <tr>
                                            <td width="160px">代表取締役</td>
                                            <td>藤田 恭嗣</td>
                                        </tr>
                                        <tr>
                                            <td>取締役</td>
                                            <td>新名 新</td>
                                        </tr>
                                        <tr>
                                            <td>取締役</td>
                                            <td>片山 誠</td>
                                        </tr>
                                        <tr>
                                            <td>取締役</td>
                                            <td>鈴木 克征</td>
                                        </tr>
                                        <tr>
                                            <td>社外取締役</td>
                                            <td>駿田 和彦</td>
                                        </tr>
                                        <tr>
                                            <td>社外取締役</td>
                                            <td>榎 啓一</td>
                                        </tr>
                                        <tr>
                                            <td>監査役</td>
                                            <td>大和田 和惠</td>
                                        </tr>
                                        <tr>
                                            <td>社外監査役</td>
                                            <td>高山 健</td>
                                        </tr>
                                        <tr>
                                            <td>社外監査役</td>
                                            <td>森藤 利明</td>
                                        </tr>
                                        <tr>
                                            <td>社外監査役</td>
                                            <td>椎名 毅</td>
                                        </tr>
                                    </table>

                                    <br />
                                    <table>
                                        <tr>
                                            <td width="160px">社長執行役員 CEO</td>
                                            <td>藤田 恭嗣</td>
                                        </tr>
                                        <tr>
                                            <td>副社長執行役員 COO</td>
                                            <td>新名 新</td>
                                        </tr>
                                        <tr>
                                            <td>上級執行役員 CFO</td>
                                            <td>片山 誠</td>
                                        </tr>
                                        <tr>
                                            <td>執行役員 CAO</td>
                                            <td>鈴木 克征</td>
                                        </tr>
                                        <tr>
                                            <td>執行役員 CBO</td>
                                            <td>溝口 敦</td>
                                        </tr>
                                    </table>
                                    <br />
                                    <small><span>※2018年12月1日時点</span></small>
                                </td>
                            </tr>
                            <tr>
                                <td>主な事業</td>
                                <td>グループ戦略の立案、各事業会社の統括管理、および各種研究開発</td>
                            </tr>
                            <tr>
                                <td>取引銀行</td>
                                <td>三井住友信託銀行／りそな銀行／三菱東京UFJ銀行／みずほ銀行／三井住友銀行／阿波銀行／徳島銀行<br /><small>※順不同</small></td>
                            </tr>
                            <tr>
                                <td>監査法人</td>
                                <td>有限責任 あずさ監査法人</td>
                            </tr>
                        </tbody>
                    </table>

                </div>

                <div id="GroupPageId102" class="tabcontentGroupPage tabcontent">
                    <div class="group-company">
                        <div class="group-company-logo">
                            <img src="<?php echo get_template_directory_uri()?>/asset/imgs/companies/svglogo_md_002.svg" alt="">
                        </div>
                    	
                    	<div class="group-company-info">
                    		<h5>株式会社メディアドゥ</h5>
                    		<ul>
                    			<li>電子書籍取次として電子書籍コンテンツを提供するとともに、電子書店や電子書籍ビジネスに関するあらゆるソリューションを提供するとともに海外に向けた輸出事業や、電子図書館事業を展開</li>
                    			<li>所在地／東京都千代田区一ツ橋1-1-1 パレスサイドビル 5F</li>
                    			<li>代表取締役社長／新名 新</li>
                    		</ul>
                            <div class="company-link">
                                <a href="https://www.mediado.jp/">サイト</a>
                            </div>
                    	</div>
                    </div>
                    <div class="group-company">
                        <div class="group-company-logo">
                            <img src="<?php echo get_template_directory_uri()?>/asset/imgs/companies/svglogo_pubridge_001.svg" alt="">
                        </div>
                        
                        <div class="group-company-info">
                            <h5>株式会社出版デジタル機構</h5>
                            <ul>
                                <li>出版業界のデジタル展開を支援するべく、電子書籍取次、POD取次、コンテンツ制作支援（Picassol）、コンテンツプロモーション（NetGalley）等、幅広く事業を展開</li>
                                <li>所在地／東京都千代田区一ツ橋1-1-1 パレスサイドビル 5F</li>
                                <li>代表取締役／新名 新</li>
                            </ul>
                            <div class="company-link">
                                <a href="https://www.pubridge.jp/">サイト</a>
                            </div>
                        </div>
                    </div>
                    <div class="group-company">
                        <div class="group-company-logo">
                            <img src="<?php echo get_template_directory_uri()?>/asset/imgs/companies/svglogo_mdi_001.svg" alt="">
                        </div>
                        
                        <div class="group-company-info">
                            <h5>Media Do International Inc.</h5>
                            <ul>
                                <li>メディアドゥグループの事業ミッションを国際的に展開し、著作者の事業機会を世界中に拡大推進</li>
                                <li>所在地／8445 Camino Santa Fe Suite 210, San Diego, CA 92121</li>
                                <li>代表取締役／塩濱 大平</li>
                            </ul>
                        </div>
                    </div>

                    <div class="group-company">
                        <div class="group-company-logo">
                            <img src="<?php echo get_template_directory_uri()?>/asset/imgs/companies/svglogo_mdtt_001.svg" alt="">
                        </div>
                        
                        <div class="group-company-info">
                            <h5>株式会社メディアドゥテック徳島</h5>
                            <ul>
                                <li>徳島の大手SIerであるテック情報グループとの合弁会社。メディアゥグループの様々なシステム開発やオペレーション業務を推進</li>
                                <li>所在地／徳島県徳島市川内町平石若松108-4 ジャストシステム徳島本社ビル 4F</li>
                                <li>代表取締役社長／藤田 恭嗣</li>
                            </ul>
                            
                        </div>
                    </div>

                    <div class="group-company">
                        <div class="group-company-logo">
                            <img src="<?php echo get_template_directory_uri()?>/asset/imgs/companies/logo_artra_002.png" alt="">
                        </div>
                        
                        <div class="group-company-info">
                            <h5>アルトラエンタテインメント株式会社</h5>
                            <ul>
                                <li>電子コミックのカラーリング、コミックの作画支援サービスの提供</li>
                                <li>所在地／東京都目黒区目黒1-6-17 8F</li>
                                <li>代表取締役社長／岡田 有正</li>
                            </ul>
                            
                        </div>
                    </div>

                    <div class="group-company">
                        <div class="group-company-logo">
                            <img src="<?php echo get_template_directory_uri()?>/asset/imgs/companies/flier.svg" alt="">
                        </div>
                        
                        <div class="group-company-info">
                            <h5>株式会社フライヤー</h5>
                            <ul>
                                <li>書籍の要約コンテンツの提供</li>
                                <li>所在地／東京都千代田区一ツ橋1-1-1 パレスサイドビル 8F</li>
                                <li>代表取締役CEO／大賀 康史</li>
                            </ul>
                            <div class="company-link">
                                <a href="https://www.flierinc.com/">サイト</a>
                            </div>
                        </div>
                    </div>

                    <div class="group-company">
                        <div class="group-company-logo">
                            <img src="<?php echo get_template_directory_uri()?>/asset/imgs/companies/svglogo_manga_001.svg" alt="">
                        </div>
                        
                        <div class="group-company-info">
                            <h5>株式会社マンガ新聞</h5>
                            <ul>
                                <li>マンガ情報メディア「マンガ新聞」の運営</li>
                                <li>所在地／東京都千代田区一ツ橋1-1-1 パレスサイドビル 8F</li>
                                <li>代表取締役社長／皆川 淳</li>
                            </ul>
                            <div class="company-link">
                                <a href="http://www.manga-news.jp/">サイト</a>
                            </div>
                        </div>
                    </div>

                    <div class="group-company">
                        <div class="group-company-logo">
                            <img src="<?php echo get_template_directory_uri()?>/asset/imgs/companies/lunascape.svg" alt="">
                        </div>
                        
                        <div class="group-company-info">
                            <h5>Lunascape株式会社</h5>
                            <ul>
                                <li>ブラウザーの開発・提供</li>
                                <li>所在地／東京都渋谷区桜丘町26-1セルリアンタワー 15F</li>
                                <li>代表取締役社長／藤田 恭嗣</li>
                            </ul>
                            <div class="company-link">
                                <a href="http://www.lunascape.co.jp/">サイト</a>
                            </div>
                        </div>
                    </div>

                    <div class="group-company">
                        <div class="group-company-logo">
                            <img src="<?php echo get_template_directory_uri()?>/asset/imgs/companies/logo_jct_001.png" alt="">
                        </div>
                        
                        <div class="group-company-info">
                            <h5>株式会社Jコミックテラス</h5>
                            <ul>
                                <li>無料電子コミックサービス「マンガ図書館Z」（<a href="https://www.mangaz.com/">https://www.mangaz.com/</a>）の運営をはじめとする、電子書籍配信業</li>
                                <li>所在地／東京都千代田区一ツ橋1-1-1 パレスサイドビル 8F</li>
                                <li>代表取締役社長／佐藤 美佳</li>
                            </ul>
                            <div class="company-link">
                                <a href="https://j-comi.co.jp/">サイト</a>
                            </div>
                        </div>
                    </div>

                    <div class="group-company">
                        <div class="group-company-logo">
                            <img src="<?php echo get_template_directory_uri()?>/asset/imgs/companies/logo_tds_001.png" alt="">
                        </div>
                        
                        <div class="group-company-info">
                            <h5>株式会社徳島データサービス</h5>
                            <ul>
                                <li>データ入力事業、健康促進事業、管理運営事業、他</li>
                                <li>所在地／徳島市沖浜東三丁目46番地 Ｊビル西館4階</li>
                                <li>代表取締役社長／濱尾 重忠</li>
                            </ul>
                            <div class="company-link">
                                <a href="http://www.tds-inf.co.jp/">サイト</a>
                            </div>
                        </div>
                    </div>

                </div>

            </div>
        </div>
    </div>
</div>
<?php get_footer();?>