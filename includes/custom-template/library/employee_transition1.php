        ...
        var row = i + location; 
				for(var j = 1; j<13; j++){
					if(response.data.values[row][j]) {
						...
						if( j === 1){
							count= response.data.values[row][j]-lastMonthValue[row];
						} else {
...
async function renderTable(){
    ...
    writeTable(response,1,"mdhd_nichi_",lastMonthValue);
		writeTable(response,14,"mdhd_matsujitsu_",lastMonthValue);
		writeTable(response,27,"md_nichi_",lastMonthValue);
		writeTable(response,40,"md_matsujitsu_", lastMonthValue);
			}).catch(function (error) { console.log(error);});
}
